    ...
    <div className="w-screen h-screen flex flex-col bg-surface-canvas font-sans text-body overflow-hidden antialiased">
      ...
      <header className="h-14 px-3 sm:px-5 lg:px-7 bg-surface-canvas/90 border-b border-hairline flex items-center justify-between shrink-0 backdrop-blur-md z-30">
        <div className="flex items-center gap-2 sm:gap-4">
          <div className="p-2 rounded-md text-primary-text ring-2 ring-primary/40 flex items-center justify-center shrink-0">
            <Workflow className="w-5 h-5" />
          </div>
          <div className="flex items-center gap-1 bg-surface-card rounded-md p-1 border border-hairline">
            <button
              onClick={() => changeTab('history')}
              className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-sm text-xs font-medium transition-all cursor-pointer ${
                activeTab === 'history'
