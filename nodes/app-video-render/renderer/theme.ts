...
/** Stable renderer-owned theme seed. The storyboard never writes a hue. */
export function getDocumentHue(document: {
  slug?: string;
  title?: string;
  summary?: string;
}): number {
  const source = `${document.slug ?? ''}|${document.title ?? ''}|${document.summary ?? ''}`;
  let hash = 0;
