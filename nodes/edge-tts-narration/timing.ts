 ...
 * bottleneck."` — and Edge TTS reports, per word, when it actually said that
 * word. Lining the two up gives shot boundaries that cannot drift from the
 * voice, because they are read off the voice.
 *
...
import type { WordBoundary } from './edge-tts.ts';
 ...
 * land in it. Whitespace is collapsed exactly as `splitTextForSynthesis` does,
 * so offsets here describe the same string Edge TTS was given.
 */
...
function evenSplit(itemCount: number, audioSeconds: number): ItemTiming[] {
  return Array.from({ length: itemCount }, (_, index) => {
    const start = (audioSeconds * index) / itemCount;
    const end = (audioSeconds * (index + 1)) / itemCount;
    return { index, startSeconds: round(start), durationSeconds: round(end - start) };
  });
}

function toTimings(switches: number[], itemCount: number, audioSeconds: number): ItemTiming[] {
  const edges = [0, ...switches, audioSeconds];
  return Array.from({ length: itemCount }, (_, index) => ({
    index,
    startSeconds: round(edges[index]),
    durationSeconds: round(edges[index + 1] - edges[index]),
  }));
}
...
export function resolveClipTiming(options: ResolveClipTimingOptions): ClipTimingResult {
  ...
  if (!offsets.length) {
    warnings.push(`${label} narration reported no word boundaries; shot timing was split evenly.`);
    return { items: evenSplit(itemCount, audioSeconds), measured: false, warnings };
  }

  const switches = anchors.map((anchor) => {
  ...
  return { items: toTimings(switches, itemCount, audioSeconds), measured: true, warnings };
}
